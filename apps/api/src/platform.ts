...
import { INTERVIEW_DATASET } from "./data/interviewDataset.js";
...
function generatePlanText(
  ...
  const lines = rounds.map(
    (r, idx) =>
      `${idx + 1}. ${r.name} — focused on ${r.focusAreas.join(", ")} with ${r.interviewers.length} interviewer(s).`,
  );
...
function blueprintFromTemplate(
  ...
  return {
    ...
    resumeHighlights: [
      "Practice session generated from a community interview template",
    ],
    evaluationCriteria: [
      "Technical Ability",
      "Problem Solving",
    ...
    rounds: template.rounds.map((r, idx) => {
      ...
      if (interviewers.length === 0) {
        interviewers.push({
          ...
          role:
            r.type === "RECRUITER" ? "Technical Recruiter" : "Staff Engineer",
          avatarUrl: AVATARS[idx % AVATARS.length],
          personality: "Professional, structured, and adaptive",
          style:
            r.type === "RECRUITER" ? "Conversational" : "Deep-dive technical",
...
function evaluateRound(session: PracticeSession): RoundResult {
  ...
  const roundTurns = session.transcripts.filter(
    (t) => t.roundIndex === roundIndex,
  );
  const candidateTurns = roundTurns.filter((t) => t.sender === "candidate");

  const avgWords =
    candidateTurns.reduce((acc, t) => acc + t.text.split(/\s+/).length, 0) /
    (candidateTurns.length || 1);
  const depthWords = candidateTurns.reduce(
  ...
  if (/scale|scalability|req\/sec|throughput|latency|p99/.test(joined))
    score += 4;
  if (/trade[- ]?off|pros and cons|alternative|consideration/.test(joined))
    score += 3;
  if (/idempoten|deadlock|race|isolation|lock|transaction/.test(joined))
    score += 3;
...
export function registerPlatformRoutes(app: express.Express) {
  ...
  app.post("/api/agents", requireAuth, (req, res) => {
    try {
      ...
      if (!name || !role) {
        return res
          .status(400)
          .json({ success: false, error: "Agent name and role are required" });
      }
      const agent: Agent = {
        id: id("agent"),
        ownerId: user.id,
  ...
  app.delete("/api/agents/:id", requireAuth, (req, res) => {
    const user = getTokenUser(req);
    const idx = agentStore.findIndex(
      (a) => a.id === req.params.id && a.ownerId === user.id,
    );
    if (idx === -1)
      return res.status(404).json({ success: false, error: "Agent not found" });
  ...
  app.post("/api/interview-templates", requireAuth, (req, res) => {
    try {
      const user = getTokenUser(req);
      const { title, company, role, level, description, mode, rounds } =
        req.body;
      if (!title || !company || !role) {
        return res.status(400).json({
          success: false,
          error: "Title, company, and role are required",
        });
      }
  ...
  app.get("/api/interview-templates", requireAuth, (req, res) => {
    ...
    if (scope === "mine") {
      ...
      items = items.filter(
        (t) => t.mode === "public" && t.status === "published",
      );
    }
  ...
  app.get("/api/interview-templates/:id", requireAuth, (req, res) => {
    ...
    if (
      ...
      !(template.mode === "public" && template.status === "published")
    ) {
      return res
        .status(403)
        .json({ success: false, error: "Template is private" });
    }
    template.views += 1;
    res.json({ success: true, template });
  });
  ...
  app.post("/api/interview-templates/:id/publish", requireAuth, (req, res) => {
    ...
    const template = templateStore.find(
      (t) => t.id === req.params.id && t.ownerId === user.id,
    );
    if (!template)
      return res
        .status(404)
    ...
    if (template.mode !== "public")
      return res.status(400).json({
        success: false,
        error: "Only public templates can be published",
      });
  ...
  app.post("/api/interview-templates/:id/rate", requireAuth, (req, res) => {
    ...
    res.json({
      success: true,
      ratingAvg: template.ratingTotal / template.ratingCount,
  ...
  app.post("/api/interview-templates/:id/vote", requireAuth, (req, res) => {
    ...
    res.json({
      success: true,
      votesUp: template.votesUp,
      votesDown: template.votesDown,
      netVotes: template.votesUp - template.votesDown,
    });
  });

  app.delete("/api/interview-templates/:id", requireAuth, (req, res) => {
    const user = getTokenUser(req);
    const idx = templateStore.findIndex(
      (t) => t.id === req.params.id && t.ownerId === user.id,
    );
    if (idx === -1)
      return res
  ...
  app.post("/api/goals", requireAuth, (req, res) => {
    try {
      ...
      if (!role) {
        return res
          .status(400)
          .json({ success: false, error: "Target role is required" });
      }
  ...
  app.get("/api/goals/:id", requireAuth, (req, res) => {
    const user = getTokenUser(req);
    const goal = goalStore.find(
      (g) => g.id === req.params.id && g.userId === user.id,
    );
    if (!goal)
  ...
  app.get("/api/practice-sessions/:id", requireAuth, (req, res) => {
    const user = getTokenUser(req);
    const session = practiceStore.find(
      (s) => s.id === req.params.id && s.userId === user.id,
    );
    if (!session)
      return res
        .status(404)
        .json({ success: false, error: "Practice session not found" });
    res.json({ success: true, session });
  });
  ...
  app.post(
    "/api/practice-sessions/:id/evaluate-round",
    requireAuth,
    (req, res) => {
      const user = getTokenUser(req);
      const session = practiceStore.find(
  ...
  app.post("/api/practice-sessions/:id/complete", requireAuth, (req, res) => {
    ...
    if (session.sourceType === "goal" && session.sourceId) {
      const goal = goalStore.find(
        (g) => g.id === session.sourceId && g.userId === user.id,
      );
      if (goal) {
        const passed = session.status === "PASSED";
        goal.completedRounds = passed
          ? goal.totalRounds
          : Math.max(0, goal.completedRounds || 0);
  ...
  app.post("/api/practice-sessions/:id/feed-clip", requireAuth, (req, res) => {
    ...
    if (session.status !== "PASSED") {
      return res.status(400).json({
        success: false,
        error: "Only passed sessions can be contributed to the feed",
      });
    }
  ...
  app.get("/api/dataset", (req, res) => {
    ...
    if (qCompany && qCompany !== "general")
      items = items.filter((d) => d.company.toLowerCase().includes(qCompany));
    if (qRole)
      items = items.filter(
        (d) =>
          d.role.toLowerCase().includes(qRole) ||
          d.level.toLowerCase().includes(qRole),
      );
    if (qLevel)
      items = items.filter((d) => d.level.toLowerCase().includes(qLevel));
  ...
  app.post(
    "/api/practice-sessions/:id/agent/start",
    requireAuth,
    async (req, res) => {
      try {
        const user = getTokenUser(req);
        ...
        const persona =
          round?.interviewers?.[requestedIndex] || round?.interviewers?.[0];

        if (!persona) {
          return res.status(404).json({
            success: false,
            error: "No interviewer available for this round",
          });
        }
        ...
        session.agents = [
          ...(session.agents || []).filter(
            (a) => a.interviewerIndex !== entry.interviewerIndex,
          ),
          entry,
        ];
        ...
        res.json({
          success: true,
          agent: entry,
          round: { id: round.id, name: round.name, type: round.type },
        });
      } catch (err: any) {
        res.status(500).json({ success: false, error: err.message });
        console.log(err);
  ...
  app.post(
    "/api/practice-sessions/:id/agent/stop",
    requireAuth,
    async (req, res) => {
      try {
        const user = getTokenUser(req);
